...
**MCP 工具仅在 QA 模式下可用**。如需使用内置工具（Read/Write/Bash 等）进行文件操作，请切换到 Agent 模式。
...
| 模式 | 可用工具 | 适用场景 |
|------|----------|---------|
| **QA 模式** | 新闻 MCP、文档 RAG | 文档问答、新闻查询、闲聊 |
| **Agent 模式** | 内置工具（Read/Write/Bash/Glob/Grep） | 文件操作、脚本执行、Skill 调试 |

**切换方式**：前端界面右上角模式选择器，选择 QA 或 Agent。
...
MCP 工具通过 LangGraph 的 QA 模式中的 `classify_intent` 节点被调用：
...
A: MCP 工具通过 QA 模式的 `classify_intent` 节点被调用，使用 LLM 进行意图识别，然后执行相应的工具。具体见上文"LangGraph 与 MCP 的集成"部分。
