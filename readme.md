...
- **固定摘要模型**：摘要生成始终使用 qwen3.5:4b，确保稳定性
...
- **摘要模型**：始终使用 qwen3.5:4b 模型生成摘要，确保稳定性
...
| 来源 | 策略 | 参数 | 说明 |
|------|------|------|------|
| 早期对话 | 摘要压缩 | qwen3.5:4b | 对话轮数 > 5 时，早期消息生成摘要 |
| 历史对话 RAG | FAISS 语义检索 | k=3 | 根据当前问题检索相关历史片段 |
...
```
[SystemMessage] 系统提示词
[SystemMessage] 之前的对话摘要：...（早期对话摘要）
[SystemMessage] 以下是相关的历史对话片段：...（RAG 检索）
[HumanMessage] 用户消息
[AIMessage] 助手消息
...（最近5轮）
[HumanMessage] 当前问题
...
FAISS 向量检索（k=4）
    ↓
检索到的文档片段拼入 system_prompt
    ↓
"你是一个文档问答助手。仅基于以下内容回答问题：[文档片段]"
```
...
### 模型使用
- 摘要生成（自动对话命名、上下文摘要）始终使用固定的 `qwen3.5:4b` 模型
- 问答功能可以使用用户选择的任意模型
- 确保已下载所需的 Ollama 模型
...
### v5.1.0
- 本地记忆持久化
  - 对话历史自动保存为 Markdown 文件
  - 服务重启后自动加载历史对话
  - 支持图片资源的持久化存储
- 历史对话 RAG 检索
  - 使用 FAISS 构建历史对话向量索引
  - 根据用户问题语义检索相关历史片段（k=3）
  - 与摘要+滑动窗口组成三层上下文加载策略
  - 渐进式披露上下文
- 模型配置优化
  - 默认对话模型改为 qwen3.5:9b
...
- 新增文件
  ...
  - `history_rag.py`: 历史对话 RAG 检索
- 存储结构
  - `conversations/`: 对话 MD 文件
  - `assets/`: 图片资源
  - `vector_stores/history/`: 历史向量索引
